...
class CallbackData:
    def __init__(self, modelvars):
        ...
        self.lastitr = -GRB.INFINITY
        self.lastnode = -GRB.INFINITY


def myCallback(model, where, *, cbdata, logfile):
    """
    Callback function. 'model' and 'where' arguments are passed by gurobipy when the callback is invoked. The other arguments must be provided via functools.partial:
    1) 'cbdata' is an instance of CallbackData, which holds the model variables and tracks state information across calls to the callback.
    ...
    if where == GRB.Callback.POLLING:
        ...
        if model.cbGet(GRB.Callback.MIPNODE_STATUS) == GRB.OPTIMAL:
            ...
            print(
                f"{itcnt:.0f} {primobj:g} {dualobj:g} {priminf:g} {dualinf:g} {cmpl:g}")
...
with gp.Env(params={"OutputFlag": 0, "Heuristics": 0}) as env, gp.read(model_file, env=env) as model, open("cb.log", "w") as logfile:
